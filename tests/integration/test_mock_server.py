...
class TestMockServer:
    ...
    def test_mock_get_request(self, mock_server):
        """测试 Mock 服务器的 GET 请求"""
    ...
    def test_mock_post_request(self, mock_server):
        ...
        with allure.step("发送 POST 请求"):
            payload = {"name": "王五", "email": "wangwu@example.com"}
        ...
        with allure.step("验证响应和调用计数"):
            ...
            # 验证端点被调用次数
            call_count = mock_server.get_call_count("POST", "/api/users")
            assert call_count == 1
    ...
    @allure.severity(allure.severity_level.NORMAL)
    @allure.title("测试请求记录")
    def test_mock_request_recording(self, mock_server):
        """测试 Mock 服务器记录请求"""
