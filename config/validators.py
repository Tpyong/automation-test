...
class ConfigValidator:
    ...
    def validate_base_url(base_url: str) -> bool:
        ...
        url_pattern = re.compile(
            r"^https?://[a-zA-Z0-9][a-zA-Z0-9-]{1,61}[a-zA-Z0-9]\.[a-zA-Z]{2,}([a-zA-Z0-9-._~:/?#\[\]@!$&\'()*+,;=.]+)?$"
        )
    ...
    @staticmethod
    def validate_playwright_browser(browser: str) -> bool:
        """
        验证 Playwright 浏览器类型
        ...
        valid_browsers = ["chromium", "firefox", "webkit"]
        if browser not in valid_browsers:
            logger.error(f"无效的 Playwright 浏览器类型: {browser}，有效值: {valid_browsers}")
            return False
    ...
    def validate_all(config: Dict[str, Any]) -> List[str]:
        ...
        # 验证 Playwright 浏览器类型
        if "PLAYWRIGHT_BROWSER" in config:
            if not ConfigValidator.validate_playwright_browser(config["PLAYWRIGHT_BROWSER"]):
                errors.append("无效的 Playwright 浏览器类型")

        # 验证 API 基础 URL
        if "API_BASE_URL" in config:
            if not ConfigValidator.validate_base_url(config["API_BASE_URL"]):
                errors.append("无效的 API 基础 URL")
        ...
        if "VIEWPORT_WIDTH" in config and "VIEWPORT_HEIGHT" in config:
            if not ConfigValidator.validate_viewport(config["VIEWPORT_WIDTH"], config["VIEWPORT_HEIGHT"]):
