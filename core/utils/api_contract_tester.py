...
class APIContractTester:
    ...
    def validate_contract(self, path: str, method: str, result: APITestResult) -> List[str]:
        ...
        if contract.response_schema and isinstance(result.response_body, dict):
            errors.extend(self._validate_response_structure(
    ...
    def run_all_contract_tests(self) -> Dict[str, List[APITestResult]]:
        """运行所有契约测试"""
        results: Dict[str, List[APITestResult]] = {
            'passed': [],
            'failed': []
        }
        ...
        for key, contract in self.contracts.items():
            ...
            # 验证契约
            contract_errors = self.validate_contract(
                contract.path,
                contract.method,
                result
            )
...
class APIPerformanceTester:
    ...
    def stress_test(
        ...
        logger.info(f"压力测试完成: 成功率 {summary.get('success_rate', 0):.1f}%, 实际 RPS: {summary.get('actual_rps', 0):.1f}")
        return summary
