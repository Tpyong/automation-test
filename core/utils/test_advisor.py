...
class TestAdvisor:
    ...
    def analyze_test_performance(self, history_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            "test_durations": avg_durations,
            "test_failure_counts": test_failure_counts
        }
    
    def analyze_module_performance(self, history_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        分析模块性能
        ...
        for module, stats in module_stats.items():
            runs = stats["runs"]
            if runs > 0:
                stats["avg_duration"] = stats["duration"] / runs
    ...
    def generate_suggestions(self, days: int = 30) -> List[Dict[str, Any]]:
        ...
        # 1. 识别执行时间较长的测试
        slow_tests = [
            (test, duration) for test, duration in performance_analysis.get("test_durations", {}).items() 
            if duration > 5.0
        ]
        ...
        # 2. 识别频繁失败的测试
        frequent_failures = [
            (test, count) for test, count in performance_analysis.get("test_failure_counts", {}).items() 
            if count >= 3
        ]
        ...
        # 3. 识别表现不佳的模块
        poor_modules = [
            (module, stats) for module, stats in module_analysis.items() 
...
"""
        ...
        logger.info(f"智能测试建议报告已生成: {report_path}")
