...
class TestMonitor:
    ...
    def end_test(self, status: str, error_message: Optional[str] = None) -> TestMetrics:
        ...
        self.current_session.test_metrics.append(self._current_test)
    ...
    def generate_performance_report(self) -> str:
        ...
        report_file = self.output_dir / f"performance_{self.current_session.session_id}.html"
...
<html>
<head>
    <title>测试性能报告 - {self.current_session.session_id}</title>
    <style>
...
<body>
    ...
    <div class="summary">
        ...
        <div class="metric">
            <div>总测试数</div>
            <div class="metric-value">{self.current_session.total_tests}</div>
        </div>
        <div class="metric">
            <div>通过</div>
            <div class="metric-value" style="color: #4CAF50;">{self.current_session.passed_tests}</div>
        </div>
        <div class="metric">
            <div>失败</div>
            <div class="metric-value" style="color: #f44336;">{self.current_session.failed_tests}</div>
        </div>
        <div class="metric">
            <div>跳过</div>
            <div class="metric-value" style="color: #FF9800;">{self.current_session.skipped_tests}</div>
        </div>
        <div class="metric">
            <div>通过率</div>
            <div class="metric-value">{self.current_session.pass_rate:.1f}%</div>
        </div>
        <div class="metric">
            <div>总耗时</div>
            <div class="metric-value">{self.current_session.duration:.2f}s</div>
        </div>
        <div class="metric">
            <div>平均耗时</div>
            <div class="metric-value">{self.current_session.avg_test_duration:.3f}s</div>
        </div>
    </div>
