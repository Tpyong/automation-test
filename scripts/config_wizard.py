...
class TestConfigWizard:
    ...
    def run(self):
        ...
        config['BASE_URL'] = self.get_valid_input(
            "请输入测试基础URL", 
            config.get('BASE_URL', 'https://demo.playwright.dev/todomvc')
        )
        ...
        config['BROWSER'] = self.get_valid_input(
            "请输入浏览器类型", 
            config.get('BROWSER', 'chromium'),
            f"({'|'.join(browser_choices)})"
        )
        
        config['HEADLESS'] = str(self.get_yes_no_input(
            "是否以无头模式运行浏览器", 
            config.get('HEADLESS', 'false').lower() == 'true'
        )).lower()
        
        config['SLOW_MO'] = self.get_valid_input(
            "请输入浏览器操作延迟（毫秒）", 
            config.get('SLOW_MO', '0'),
            "^\d+$")
        ...
        config['API_BASE_URL'] = self.get_valid_input(
            "请输入API基础URL", 
            config.get('API_BASE_URL', 'https://api.example.com')
        )
        
        config['API_TIMEOUT'] = self.get_valid_input(
            "请输入API超时时间（毫秒）", 
            config.get('API_TIMEOUT', '30000'),
            "^\d+$")
        
        # 录屏配置
        print("\n=== 录屏配置 ===")
        config['VIDEO_ENABLED'] = str(self.get_yes_no_input(
            "是否启用录屏功能", 
            config.get('VIDEO_ENABLED', 'false').lower() == 'true'
        )).lower()
        ...
        config['TEST_ENV'] = self.get_valid_input(
            "请输入测试环境", 
            config.get('TEST_ENV', 'development'),
            f"({'|'.join(env_choices)})"
        )
